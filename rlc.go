...
// An RLC represents a bitmap based adaptive rate limiter.
// The quota management algorithm is based on a Round-robin bitmap tape with a moving head.
//
// An RLC is safe for use by multiple goroutines simultaneously.
//
...
// NewRLC allocates and returns a new RLC rate limiter.
//
// len is the size of the bitmap.
// reserved	is the number of reserved positive bits.
// offset is the RLC.Offset strategy instance.
func NewRLC(size int, reserved int, offset Offset) *RLC {
...
func (eq *RLC) Notify(value bool, n uint) {
	eq.Lock()
	defer eq.Unlock()
